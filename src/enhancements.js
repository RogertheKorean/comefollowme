...
function recordFor(key,language=reference.language,revision=null){
 ...
 const d=docs.at(-1);
 if(d){const rows={};d.blocks.forEach((b,i)=>{rows[i+1]={...b,text:plain(b.markdown)};});return {key,kind:d.coverage==='summary'&&d.kind==='scripture'?'article':d.kind,title:d.title,author:d.author,date:d.date,language,sourceLanguage:d.sourceLanguage,coverage:d.coverage,edition:d.edition,revision:d.revision,source:d.url,provenance:d.provenance,rows,document:d};}
 ...
 return {key,kind:'scripture',title:refBookName(key,language)+' '+s.chapter+(language==='ko'?'장':''),language,coverage:s.coverage==='full'?'full':'excerpt',edition:s.edition[language],revision:s.revision,source:s.source[language],rows,provenance:s.provenance?.[language]||(language==='ko'?'v2에 수록된 개역한글. 공과의 개역개정과 표현이 다를 수 있습니다.':'KJV scripture text retained from v2.')};
}
...
function referenceOriginalLink(){
 ...
 const sourceLanguage=currentRecord()?.sourceLanguage||reference.language;
 if(official)url.searchParams.set('lang',sourceLanguage==='ko'?'kor':'eng');
 return `<a class="reference-original" data-explicit-external="true" href="${esc(url.href)}" target="_blank" rel="noopener noreferrer"><span>${icon('external','small')}${official?E('공식 원문 열기','Open official source'):E('원문 사이트 열기','Open original source')}</span><small>${sourceLanguage==='ko'?'한국어':'English'} · ${E('새 탭','New tab')}</small></a>`;
}
function renderReference(){
 if(!reference.open)return;ensureReferenceShell();const b=resolveReference(),nums=b?Object.keys(b.rows).map(Number):[],isScript=(b?.kind||reference.kind)==='scripture';
 ...
 const caveat=status==='summary'?`<div class="reference-coverage summary-notice"><strong>${E('학습 요약·자료 안내 · 공식 전문은 원문에서','Study summary / source guide · full text at source')}</strong><p>${E('아래는 출처를 참고한 짧은 학습 안내입니다. 문단 번호는 안내 내부 위치이며 공식 원문의 문단 번호가 아닙니다.','This short reading guide has its own paragraph positions, distinct from the original source.')}</p></div>`:status==='excerpt'?`<div class="reference-coverage">${E('발췌만 수록되어 있습니다. 장·말씀·교재 전체가 아닙니다.','An excerpt is included, not the entire chapter or document.')}</div>`:'';
