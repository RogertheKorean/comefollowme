...
if(cloud.enabled){
 ...
 toggleLike=async function(id){const n=state.notes.find(n=>n.id===id);if(!n?.remote||n.scope!=='class'||likeLocks.has(id))return;likeLocks.add(id);try{const user=await ensureParticipant();const liked=n.likes.includes('me');const result=liked?await cloud.client.from('together_reactions').delete().eq('note_id',id).eq('owner_id',user.id):await cloud.client.from('together_reactions').insert({note_id:id,owner_id:user.id});if(result.error&&result.error.code!=='23505')throw result.error;if(cloud.user?.id!==user.id)return;const current=state.notes.find(n=>n.id===id);if(current)current.likes=liked?current.likes.filter(x=>x!=='me'):[...new Set([...current.likes,'me'])];refreshPreservingScroll();if(current&&modalKind==='thread'){$('.modal .note-card')?.replaceWith(document.createRange().createContextualFragment(noteCard(current)));}cloud.lastJSON='';}catch(e){toast(readableError(e),true);}finally{likeLocks.delete(id);if(!cloud.ready)void sync();}};
